...
- **`apple-health.js`** — read-only Apple Health cache used by `gym.html`. It first renders `apple_health_metrics_v1` from localStorage, then refreshes the current local date from authenticated same-origin `GET /api/sync-health` when online, using `localStorage.apple_health_sync_token`. It does not run background automation or upload Apple Health data.
- **`sync-service.js`** — explicit, opt-in browser client for POSTing sanitized Apple Health payloads to the private Supabase `sync-health` Edge Function. It never contains the service-role key and does not schedule or retry in the background.
- **`supabase/functions/sync-health/index.ts`** — private Deno Edge Function that verifies `APPLE_HEALTH_SYNC_TOKEN`, recursively sanitizes Health Auto Export samples, deduplicates stable `sample_id` values, and upserts directly into `public.health_data` through `SUPABASE_SERVICE_ROLE_KEY`. It has no GitHub/archive integration. `supabase/config.toml` disables automatic JWT verification because the function performs its own token check.
- **`api/sync-health.js`** — validates JSON from the iOS Shortcut and routes flat or Health Auto Export v2 records to the server-side writer. `lib/health-validation.js` recursively sanitizes `metadata`; `lib/supabase.js` enforces the same sanitization immediately before every `apple_health_logs` upsert with the server-only `SUPABASE_SERVICE_ROLE_KEY`. Never expose that key in frontend code. The Edge Function is an optional alternate ingestion route with the same token/schema contract.
...
- **Never commit `.env` or the real credentials** — only the placeholder source. The anon key is publishable by design (RLS is the real gate), but the source should still stay clean. `SUPABASE_SERVICE_ROLE_KEY` and `APPLE_HEALTH_SYNC_TOKEN` are server-only and must never be injected into `dist/` or shared in the Shortcut URL beyond the Authorization header. Health Auto Export samples use a stable `sample_id` when possible so retries are idempotent. The canonical database metadata column is `metadata` (`jsonb`), not `workout_metadata`; sanitize it during validation and again immediately before every Supabase write.
