...
- **`supabase/functions/sync-health/index.ts`** — private Deno Edge Function that verifies `APPLE_HEALTH_SYNC_TOKEN`, recursively sanitizes payloads, deduplicates stable external IDs, and upserts through `SUPABASE_SERVICE_ROLE_KEY`. `supabase/config.toml` disables automatic JWT verification because the function performs its own token check.
