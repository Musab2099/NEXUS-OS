...
// strings. Keep the complete structure, but ensure every leaf is JSON-safe so
// PostgREST/Postgres never receives an unsupported value.
function sanitizeMetadata(value, seen) {
  ...
  if (Array.isArray(value)) {
    const result = value.map((item) => sanitizeMetadata(item, visited));
  ...
  Object.entries(value).forEach(([key, item]) => {
    ...
    Object.defineProperty(result, String(key), {
      configurable: true,
      enumerable: true,
      value: sanitizeMetadata(item, visited),
      writable: true,
    });
  });
  visited.delete(value);
