...
const { sanitizeMetadata } = require('./health-validation');
...
function sanitizeHealthRecord(row) {
  if (!row || typeof row !== 'object' || Array.isArray(row)) return row;

  return {
    ...row,
    // Keep this guard immediately before insertion so every caller of this
    // module gets database-safe metadata, even if it bypasses route validation.
...
async function upsertHealthRecords(payloads, nested) {
  ...
  const rawRows = Array.isArray(payloads) ? payloads : [payloads];
  const rows = rawRows.map(sanitizeHealthRecord);
  const batches = [];
